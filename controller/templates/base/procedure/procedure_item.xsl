		...
		<div id="details">
			<form action="#" method="post" id="form">
				...
				<dl class="proplist-col">
					<dt>
						<label for="title">
							<xsl:value-of select="php:function('lang','Procedure title')" />
						</label>
					</dt>
					<dd>
						<xsl:choose>
					...
					<dt>
						<label for="start_date">
							<xsl:value-of select="php:function('lang','Procedure start date')" />
						</label>
					</dt>
					<dd>
						<xsl:choose>
					...
					<dd>
						<xsl:choose>
							<xsl:when test="editable">
								<textarea id="purpose" name="purpose" rows="5" cols="60" style="width: 690px;">
					...
					<dd>
						<xsl:choose>
							<xsl:when test="editable">
								<textarea id="responsibility" name="responsibility" rows="5" cols="60">
					...
					<dd>
						<xsl:choose>
							<xsl:when test="editable">
								<textarea id="reference" name="reference" rows="5" cols="60">
	...
	<xsl:for-each select="values" >
		<tr>
			<xsl:for-each select="row" >
				...
				<td>
					<a href="{$proc_link}">
						<xsl:value-of select="revision_no"/>
					</a>
				</td>
				<td>
